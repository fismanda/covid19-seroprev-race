...
# Compute BIC manually (mle2 objects from custom likelihoods don't store N,
# so BIC() returns NA; we supply N = total observations explicitly)
n_obs <- 193  # 96 White + 97 racialized after exclusion of erroneous observation
sat_bic <- -2 * sat_ll + sat_k * log(n_obs)
pars_bic <- -2 * pars_ll + pars_k * log(n_obs)
